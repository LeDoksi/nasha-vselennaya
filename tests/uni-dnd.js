...
function makeEl(tag) {
  const el = {
    ...
    classList: {
      _s: new Set(),
      add(...cs) {
        for (const c of cs) if (c) this._s.add(c);
      },
      remove(...cs) {
        for (const c of cs) this._s.delete(c);
      },
      toggle(c, f) {
        if (f === undefined ? !this._s.has(c) : f) this._s.add(c);
    ...
    setAttribute(k, v) {
      ...
      if (k.startsWith('data-')) this.dataset[toCamel(k.slice(5))] = String(v);
    },
    removeAttribute(k) {
      delete this.attrs[k];
      if (k.startsWith('data-')) delete this.dataset[toCamel(k.slice(5))];
    },
    getAttribute(k) {
...
function parseAttrs(el, str) {
  ...
  while ((a = ATTR_RE.exec(str))) {
    ...
    if (name === 'class') {
      for (const c of val.split(/\s+/).filter(Boolean)) el.classList.add(c);
    } else if (name === 'id') {
      el.id = val;
      registry['#' + val] = el;
    } else if (name.startsWith('data-')) {
      el.attrs[name] = val;
      el.dataset[toCamel(name.slice(5))] = val;
...
const document = {
  ...
  title: '',
  _handlers: {},
  _efp: null, // элемент, который «отдаёт» elementFromPoint в тесте (null — метод отсутствует)
  addEventListener(t, fn) {
    (this._handlers[t] = this._handlers[t] || []).push(fn);
  },
  removeEventListener() {},
...
assert(
  sandbox.db.notes.find(n => n.id === 'b').order === 0 && sandbox.db.notes.find(n => n.id === 'c').order === 1 && sandbox.db.notes.find(n => n.id === 'a').order === 2,
...
assert(JSON.stringify(lWrap.children.filter(c => c.classList.contains('list-card')).map(c => c.dataset.id)) === '["l1","l2","l3"]', 'карточки списков отрисованы в порядке l1,l2,l3');
...
assert(
  JSON.stringify(
    listUl()
      .children.filter(c => c.dataset.item)
...
assert(JSON.stringify(sandbox.db.lists.find(l => l.id === 'l1').items.map(i => i.id)) === '["i2","i3","i1"]', 'subtaskSortEnd: список подзадач переставлен по DOM-порядку');
...
assert(JSON.stringify(sandbox.db.lists.find(l => l.id === 'l1').items.map(i => i.id)) === '["i2","i3","i1"]', 'subtaskSortEnd: несовпадение числа детей — список подзадач не обрезается');
...
assert(JSON.stringify(sandbox.db.lists.find(l => l.id === 'l1').items.map(i => i.id)) === '["i3","i1","i2"]', 'toggleSubtask сохраняет ручной порядок внутри групп (i2 done — в конец)');
...
sandbox.photosSortEnd({
  to: { children: [photo('p2'), photo('p1')] },
  item: photo('p1'),
  from: { children: [photo('p2'), photo('p1')] },
  oldIndex: 0,
  originalEvent: { clientX: 999, clientY: 999, target: emptyTarget }
});
assert(
  sandbox.db.photos.find(p => p.id === 'p1').order === 1 && sandbox.db.photos.find(p => p.id === 'p2').order === 0,
...
fromList.insertBefore = (node, ref) => {
  // …insertBefore возвращает его на oldIndex=0
