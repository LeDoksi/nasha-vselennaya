/* Yandex Cloud Function: выдаёт подписанную (presigned) ссылку на операции с
   Yandex Object Storage — GET/PUT/DELETE одного объекта, LIST (листинг
   бакета) и batch (POST с телом {items:[{part,id}]}, до 100 штук за раз —
   фоновая очередь миниатюр не делает по одному HTTP-запросу на подпись на
   каждое фото). Секретный ключ живёт ТОЛЬКО здесь, в переменных окружения
   функции — в браузер он никогда не попадает.

   Почему это вообще нужно: бакет закрыт от анонимного доступа целиком (см.
   README, раздел B2) — Yandex Object Storage не даёт анонимно ни читать
   (GetObject/ListBucket), ни писать (PutObject/DeleteObject), даже если
   политика бакета формально это разрешает — для любой операции нужен
   подписанный запрос. Эта функция и есть тот минимальный «сервер», который
   держит секрет и подписывает запрос вместо клиента — и для чтения, и для
   записи одинаково.

   Как это используется (src/95-photos-cloud.js, makeCloudStorage): клиент делает
   ...
   чужие фото в бакете (а с NV-7, когда стало подписываться и чтение, — ещё и
   прочитать шифртекст кого угодно). Firebase Admin SDK сюда не тащим
   (тяжёлая зависимость ради одной проверки JWT) — вместо этого руками
   проверяем подпись Firebase ID-токена по публичным сертификатам Google
   (RS256, тот же алгоритм, что и внутри Admin SDK). Анонимный вход не даёт
   «настоящей» личности (это и не нужно — оба партнёра всё равно
   равноправны), но требует пройти через Firebase Auth, а не просто знать
   URL функции.
